...
# Holt sich den Token absolut sicher aus den Space-Secrets
if "HF_TOKEN" in st.secrets:
    HF_TOKEN = st.secrets["HF_TOKEN"]
    client = InferenceClient(token=HF_TOKEN)
else:
    st.error("❌ HF_TOKEN wurde nicht in den Space-Settings gefunden! Bitte lege das Secret an.")
    st.stop()

st.title("🏑 HockeyAI Studio")
st.caption("Feldhockey Content Creator | FLUX Ultra-Version 🚀")
...
with tab1:
    ...
    with col1:
        ...
        opponent = st.text_input("Gegner", "Mannheimer MHC")
    with col2:
...
st.caption("HockeyAI Studio • Sicherer Secret-Modus mit FLUX & Llama")
