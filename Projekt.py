...
# Der Nutzer kann hier seinen Token eintragen. Wenn leer, wird ein Standard-Client ohne Token probiert.
user_token = st.sidebar.text_input("Hugging Face Token (optional)", type="password", help="Ohne Token kann es zu Timeouts oder Limitierungen kommen.")

if user_token:
    client = InferenceClient(token=user_token)
    st.sidebar.success("🔒 Eigener Token aktiv!")
else:
    client = InferenceClient() # Versucht den Zugriff ohne Token (öffentlich)
    st.sidebar.warning("⚠️ Kein Token eingetragen. Nutze öffentliche API (stark limitiert).")

# --- APP HEADER ---
st.title("🏑 HockeyAI Studio")
st.caption("Feldhockey Content Creator • Premium Edition")

# Globale Standardwerte definieren, damit Tab 3 nicht abstürzt
...
with tab1:
    ...
    if st.button("🚀 4 Stories generieren", type="primary", use_container_width=True):
        with st.spinner("Generiere Hockey-Bilder..."):
            ...
            # Ohne Token nutzen wir stabilere, freie Modelle (z.B. Stable Diffusion)
            model_to_use = "stabilityai/stable-diffusion-xl-base-1.0" if not user_token else "black-forest-labs/FLUX.1-schnell"
            ...
            for i in range(4):
                ...
                try:
                    ...
                    st.error(f"Story {i+1} konnte nicht geladen werden. Grund: {str(e)[:100]}")
...
with tab3:
    ...
    if st.button("Ideen generieren"):
        with st.spinner("Llama denkt nach..."):
            # Nutzt aktuelle Session-State Werte, falls in Tab 1 was geändert wurde
            current_score = st.session_state["gameday_score"]
            current_opponent = st.session_state["gameday_opponent"]
            
            # Ein kleineres, oft frei zugängliches Llama-Modell oder Standard-Textgen
...
st.caption("HockeyAI Studio • Token-Free / Flex-Token Version")
